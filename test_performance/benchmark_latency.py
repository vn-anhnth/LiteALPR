...
def benchmark_pipeline_dataset(
    image_dir,
    det_model_path=None,
    rec_model_path=None,
    num_samples=1000,
    ...
    if det_model_path is None:
        det_model_path = os.path.join(
            litealpr_root, "output", "det", "yolov8n_efficient", "train", "weights", "best_416.onnx"
        )
    if rec_model_path is None:
        rec_model_path = os.path.join(
            litealpr_root, "output", "rec", "svtr26_tiny", "train", "best.onnx"
        )
    ...
    if len(all_files) < num_samples:
        print(
            f"Warning: Found {len(all_files)} images, which is less than requested {num_samples}."
        )
    ...
    # Initialize LiteALPR
    alpr = LiteALPR(det_model_path=det_model_path, rec_model_path=rec_model_path, device=device)
    ...
    for pass_idx in range(1, num_runs + 1):
        ...
        for img in pbar:
            ...
            det_results = alpr.det_model(
                img, verbose=False, conf=0.25, imgsz=416, device=device_arg
            )[0]
        ...
        print(
            f"Pass {pass_idx}/{num_runs}: Pre={np.mean(pass_pre):.2f}ms | DetInfer={np.mean(pass_det_infer):.2f}ms | RecInfer={np.mean(pass_rec_infer):.2f}ms | Infer={np.mean(pass_total_infer):.2f}ms | Post={np.mean(pass_post):.2f}ms | Total={np.mean(pass_total):.2f}ms | FPS={pass_fps:.2f}"
        )
    ...
    print("\n" + "=" * 72)
    print(
        f" SUMMARY BENCHMARK RESULTS ON {device.upper()} (Mean ± Std across {num_runs} passes x {len(images)} images)"
    )
    print("=" * 72)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--images_dir",
        type=str,
        default=os.path.join(litealpr_root, "dataset", "det", "train", "images"),
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            litealpr_root, "pretrained_models", "det", "yolov8n_efficient", "best_416.onnx"
        ),
    ...
    parser.add_argument(
        "--rec_model_path",
        type=str,
        default=os.path.join(litealpr_root, "pretrained_models", "rec", "svtr26_tiny", "best.onnx"),
        help="Path to the recognition model",
    )
    parser.add_argument(
        "--device", type=str, default="cuda:0", help="Device to use (e.g. 'cpu' or 'cuda:0')"
    )
