...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--model",
        type=str,
        default="output/rec/svtr26_tiny/train/best.pth",
        help="Path to model .pth",
    )
    parser.add_argument(
        "--save_path",
        type=str,
        default="output/rec/svtr26_tiny/train/best.onnx",
...
def main():
    ...
    print(
        f"[INFO] Exporting to ONNX (input={args.imgH}x{args.imgW}, opset={opset_version}, FP32, dynamic={args.dynamic})..."
    )
    torch.onnx.export(
        model,
        dummy_input,
