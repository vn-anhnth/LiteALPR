...
def param_groups_weight_decay(model: nn.Module, weight_decay=1e-5, no_weight_decay_list=()):
    ...
    for name, param in model.named_parameters():
        ...
        if (
            ...
            or any(nd in name for nd in no_weight_decay_list)
        ):
...
def build_optimizer(optim_config, lr_scheduler_config, epochs, step_each_epoch, model):
    ...
    if isinstance(model, nn.Module):
        ...
        filter_bias_and_bn = (
            config.pop("filter_bias_and_bn") if "filter_bias_and_bn" in config else False
        )
        if weight_decay > 0.0 and filter_bias_and_bn:
            no_weight_decay = {}
            if hasattr(model, "no_weight_decay"):
                no_weight_decay = model.no_weight_decay()
            parameters = param_groups_weight_decay(model, weight_decay, no_weight_decay)
    ...
    optim = getattr(torch.optim, config.pop("name"))(params=parameters, **config)

    lr_config = copy.deepcopy(lr_scheduler_config)
    scheduler_name = lr_config.pop("name")

    lr_config.update({"epochs": epochs, "step_each_epoch": step_each_epoch, "lr": config["lr"]})
