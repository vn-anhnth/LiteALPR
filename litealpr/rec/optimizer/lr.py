...
class StepLR:
    def __init__(
        self, step_each_epoch, step_size, warmup_epoch=0, gamma=0.1, last_epoch=-1, **kwargs
    ):
        super().__init__()
...
class MultiStepLR:
    def __init__(
        self, step_each_epoch, milestones, warmup_epoch=0, gamma=0.1, last_epoch=-1, **kwargs
    ):
        super().__init__()
...
class WarmupCosineLR:
    def __init__(
        self, epochs, step_each_epoch, warmup_steps=0, eta_min=0.0, last_epoch=-1, **kwargs
    ):
        super().__init__()
    ...
    def __call__(self, optimizer):
        ...
        if self.warmup_steps > 0:
            warmup_scheduler = lr_scheduler.LinearLR(
                optimizer, start_factor=1e-7, end_factor=1.0, total_iters=self.warmup_steps
            )
        ...
        cosine_scheduler = lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=remain_steps, eta_min=self.eta_min
        )
        ...
        if len(schedulers) == 1:
            ...
            return lr_scheduler.SequentialLR(
                optimizer, schedulers=schedulers, milestones=milestones, last_epoch=self.last_epoch
            )
