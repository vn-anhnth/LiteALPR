...
class EfficientRCTCDecoder(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels=38, bottleneck_channels=128, return_feats=False, **kwargs
    ):
        super().__init__()

        # 1x1 Channel Fusion (Bottleneck) to reduce parameters in Classifier
        self.fusion = nn.Conv2d(in_channels, bottleneck_channels, kernel_size=1, bias=True)
